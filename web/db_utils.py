...
engine = sqla.create_engine(
    "mysql+mysqlconnector://{}:{}@{}:{}/{}".format(
        USER, data["DB_PASSWORD"], URI, PORT, DB
    ),
    echo=True,
)
...
class DBRow:
    ...
    def __str__(self):
        return ", ".join(
            [str(self.__getattribute__(column)) for column in self.columns]
        )
...
def insert_rows(rows: list[DBRow], table: sqla.Table):
    with engine.connect() as conn:
        conn.execute(sqla.insert(table), [r.values() for r in rows])
...
def get_stations():
    with engine.connect() as conn:
        ...
        for row in rows:
            ...
            ).values()  # Convert the list into a StationRow instance
...
def get_availabilities():
    with engine.connect() as conn:
        ...
        subq = (
            select(
                table.c.StationId, func.max(table.c.LastUpdated).label("LastUpdated")
            )
            .group_by(table.c.StationId)
            .subquery()
        )
        stmt = select(
            table.c.StationId,
            ...
            subq,
            sqla.and_(
                table.c.StationId == subq.c.StationId,
                table.c.LastUpdated == subq.c.LastUpdated,
            ),
        )
...
def get_historical_availabilities(station_id, days):
    with engine.connect() as conn:
        ...
        stmnt = (
            select(table)
            .where(table.c.StationId == station_id)
            .where(func.DATE(table.c.LastUpdated) >= min_date)
        )
...
def delete_availabilities(station_id: int, start_timestamp: int, end_timestamp: int):
    with engine.connect() as conn:
        ...
        table = AvailabilityRow.table
        result = conn.execute(
            sqla.delete(AvailabilityRow.table).where(
...
def get_updated_rows(pending_rows: list[DBRow]):
    ...
    for pending_row_idx in pending_rows:
        ...
        if k not in existing_rows_by_id.keys():  # This is a brand new row
            ...
            continue
        if (
            existing_rows_by_id[k] != pending_rows[k]
        ):  # If it does exist and the rows mismatch, we need to update
...
def get_current_weather():
    with engine.connect() as conn:
        current_weather = []
        stmnt = (
            select(CurrentWeatherRow.table)
            .order_by(desc(CurrentWeatherRow.table.c.DateTime))
            .limit(1)
        )
        rows = conn.execute(stmnt)

        for row in rows:
            currentweather = CurrentWeatherRow(
                list(row), is_sql=True
            ).values()  # Convert the list into a CurrentWeatherRow instance
...
def get_historical_weather(days):
    with engine.connect() as conn:
        historical_weather = []
        min_date = datetime.datetime.now() - datetime.timedelta(days)
        stmnt = select(CurrentWeatherRow.table).where(
            func.DATE(CurrentWeatherRow.table.c.DateTime) >= min_date
        )
        rows = conn.execute(stmnt)
...
def get_weather_forecast(forecast_dt: int):
    with engine.connect() as conn:
        dt_object = datetime.datetime.fromtimestamp(forecast_dt).replace(
        ...
        stmnt = (
            select(table)
            .where(table.c.ForecastDate == dt_object)
