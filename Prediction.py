...
class Prediction:
    ...
    def _train_model(self):
        for prepared_df in self._prepared_df_list:
            station_number = int(prepared_df['id'][0])
            print(f'start training station {station_number}')
            train_input_df = prepared_df.drop(['weather_data',
                                               'id', 'mechanical_available','ElectricBikesAvailable'
                                               'StandsAvailable'], axis=1)
            # train_input_df = prepared_df[['hour', 'day', 'day_time']]
            train_output_df = prepared_df['MechanicalBikesAvailable','ElectricBikesAvailable' ]
